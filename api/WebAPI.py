...
def create_app(conductor):
    ...
    # Initialize SocketIO (let library choose best async mode)
    ...
    @app.route('/api/animations/reload', methods=['POST'])
    def reload_animations():
        """Reload animations from disk without restarting."""
        try:
            names = app.conductor.reload_animations()
            return jsonify({"status": "ok", "animations": names})
        except Exception as e:
            logger.error(f"Error reloading animations: {e}")
            return jsonify({"status": "error", "message": str(e)}), 500
