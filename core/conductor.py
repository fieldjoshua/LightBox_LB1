...
from .performance import FrameBufferPool, FrameRateLimiter, PerformanceMonitor
...
class AnimationProgram:
    ...
    def __init__(self, name: str, animate_func: Callable, params: dict | None = None):
        self.name = name
...
class Conductor:
    ...
    def __init__(self, config_path_or_manager: str | ConfigManager = "config/settings.json"):
        # Core components
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            default_animation = self.config.get("animation_program", "fallback")
    ...
    def _load_animations(self):
        ...
        # Ensure at least one built-in fallback animation exists
        if not self.animations:
            def _fallback_animate(pixels, config, frame):
                width = int(config.get("hub75.cols", 64))
                height = int(config.get("hub75.rows", 64))
                t = frame % 255
                color = (t, 255 - t, (t * 2) % 255)
                for i in range(min(len(pixels), width * height)):
                    pixels[i] = color
            self.animations["fallback"] = AnimationProgram("fallback", _fallback_animate, {})
            logger.warning("No animations found on disk; using built-in fallback animation")
    ...
    def reload_animations(self) -> list[str]:
        """Reload animations from disk and keep a valid current animation."""
        with self._animation_lock:
            self.animations.clear()
            self._load_animations()
            current = self.config.get("animation_program")
            if current not in self.animations and self.animations:
                # fallback to first available
                first = next(iter(self.animations.keys()))
...
def main():
    ...
    if conductor.config.get("enable_web", True):
        try:
            # Import the simple web app helpers
            from web.app_simple import create_app, run_server  # type: ignore[reportMissingImports]
